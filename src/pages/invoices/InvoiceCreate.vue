...
const client = ref<string | number>('')
const invoiceDate = ref('')
const dueDate = ref('')
const notes = ref('')
const items = ref<InvoiceItem[]>([{ description: '', qty: 1, price: 0 }])
  ...
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <div class="flex items-center gap-3">
        ...
        <div>
          <h1 class="text-xl font-bold">Create Invoice</h1>
          <p class="text-sm text-gray-500 mt-1">Create a new invoice for your client</p>
        </div>
      </div>
      <div class="flex gap-2">
        <BaseButton variant="ghost" @click="router.back()">Cancel</BaseButton>
        <BaseButton :icon="Save" @click="saveInvoice">Save Invoice</BaseButton>
      </div>
    </div>
